...
/////////////////////////////////////////////////////////////////////////////////////
#define LANDSCAPE        // 0: portrait,   1: landscape
//#define PORTRAIT        // 0: portrait,   1: landscape

// ---------- Graph 좌표(L), outline 크기 등 ----------
...
// ---------- Graph 좌표(P), outline 크기 등 ----------
#elif defined(PORTRAIT)   // HW Ver.2 : S1(황_21), S2(녹_19), Key(보_4)
#define SCREEN_ROTATION 0
#define GRAPH_X 40
#define GRAPH_Y 66  //64
#define GRAPH_W 120//240 //260           // 그래프박스 너비
#define GRAPH_H 90
...
#define DOUBLE_CLICK_MS 350             // 더블클릭, 일반적인 더블클릭 판정 시간 (400ms)
...
void drawTimeTempHumi(float t, float h, bool timeAvailable) {
  ...
  struct tm timeinfo;
  // getLocalTime의 두번째 인자는 타임아웃(ms) 입니다. 0으로 설정하면 non-blocking으로 즉시 상태만 확인합니다.
  bool isTimeValid = getLocalTime(&timeinfo, 0) && timeinfo.tm_year > (2020 - 1900);

  if (isTimeValid) {
    if (!timeSynced) {
        timeSynced = true; // 상태 업데이트
        lastNtpSync = millis();
    }
    tft.setTextSize(2);
    ...
    // 시간이 유효하지 않은 경우, 진단 메시지 표시
    if (timeSynced) {
        timeSynced = false; // 동기화 상태였으나 끊김
    }
    tft.setTextSize(1);
...
void ntpUpdate() {
    unsigned long now = millis();
    // timeSynced가 아직 false이거나(최초 동기화), 마지막 동기화 후 24시간이 지났을 경우 동기화 시도
    if (!timeSynced || (now - lastNtpSync >= NTP_SYNC_INTERVAL)) {
        
        if (WiFi.status() != WL_CONNECTED) {
            // WiFi가 연결되지 않았으면, 시도하지 않고 다음 loop에서 다시 확인
            return;
        }

        // NTP 서버 설정 및 시간 동기화 시도 (폴백 서버 추가하여 안정성 향상)
        configTime(gmtOffset_sec, daylightOffset_sec, ntpServer, "pool.ntp.org");
        struct tm timeinfo;
        
        // getLocalTime은 내부적으로 타임아웃(기본 5초)을 가짐.
        if (getLocalTime(&timeinfo)) {
            // getLocalTime이 성공하더라도, 유효한 시간인지 확인 (예: 1970년이 아닌지)
            if (timeinfo.tm_year > (2020 - 1900)) {
                timeSynced = true;
                lastNtpSync = now; // 성공한 시간을 기준으로 다음 동기화 시간 계산
            }
        }
        // else: 동기화 실패 시, timeSynced는 false로 유지되며 다음 주기에 재시도.
    }
}
...
void setup() {
  ...
  if (savedNtpMode == "builtin1") {
    ...
    if (savedCustomNtp.length() > 0) {
      savedCustomNtp.toCharArray(ntpServer, sizeof(ntpServer));
    } else {
      // 'custom'이 선택되었지만 주소가 비어있는 경우, 기본값으로 대체
      strcpy(ntpServer, "time.bora.net");
    }
  }
  ...
  tft.init();
  tft.setRotation(SCREEN_ROTATION);     // 0: Portrait, 1: Landscape
  tft.fillScreen(BG_COLOR);
  ...
  // Time sync will be handled in the main loop by ntpUpdate()
