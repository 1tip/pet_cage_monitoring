...
const long gmtOffset_sec = 9*3600; // 한국 시간
...
void drawTempHumi(float t, float h, bool err, bool timeAvailable) {
  // 표시 영역 유지
  ...
  char buffer[32];
  if (!timeAvailable) {
    snprintf(buffer, sizeof(buffer), "--/-- --:--:-- T:%02.0f, H:%02.0f", t, h);
  } else {
    struct tm timeinfo;
...
void loop() {
  ...
  if (millis() - lastSample >= SAMPLE_INTERVAL) {
    ...
    drawTempHumi(t, h, err, externalAPConnected);
