// 그리프와 격자 겹침발생시 오류 수정 (그래프가 그리드에 묻힘) + 그래프 구조개선
...
void computeTimeAxis(TimeAxisState &axis) {
  ...
  float anchorOffsetPx =
      (axis.nowMin - axis.anchorMin) / axis.minPerPx;

  axis.anchorX =
      GRAPH_X + GRAPH_W - (int)(anchorOffsetPx + 0.5f);
}
...
void drawGraphData() {
  for (int i = 1; i < GRAPH_W; i++) {
    if (!isnan(humiBuf[i - 1]) && !isnan(humiBuf[i])) {
      int y1 = map(humiBuf[i - 1], 0, 80,
                   GRAPH_Y + GRAPH_H, GRAPH_Y);
      int y2 = map(humiBuf[i], 0, 80,
                   GRAPH_Y + GRAPH_H, GRAPH_Y);
      tft.drawLine(GRAPH_X + i - 1, y1,
                   GRAPH_X + i, y2, TFT_GREEN);
    }
  }

  for (int i = 1; i < GRAPH_W; i++) {
    if (!isnan(tempBuf[i - 1]) && !isnan(tempBuf[i])) {
      int y1 = map(tempBuf[i - 1], 0, 80,
                   GRAPH_Y + GRAPH_H, GRAPH_Y);
      int y2 = map(tempBuf[i], 0, 80,
                   GRAPH_Y + GRAPH_H, GRAPH_Y);
      tft.drawLine(GRAPH_X + i - 1, y1,
                   GRAPH_X + i, y2, TFT_YELLOW);
    }
  }
}
