...
.container{max-width:600px;margin:2em auto;background-color:#fff;padding:1.5em;border-radius:8px;box-shadow:0 4px 8px rgba(0,0,0,0.1)}
h2{text-align:center;color:#007bff;margin-bottom:1em}
.menu-button{display:block;width:100%;padding:12px;margin-bottom:10px;background-color:#007bff;color:#fff;text-decoration:none;border-radius:4px;text-align:center;font-weight:bold;box-sizing:border-box}
.menu-button:hover{background-color:#0056b3}
.download-button{background-color:#28a745;margin-top:20px}
.download-button:hover{background-color:#218838}
.chart-container{margin-top:30px;border:1px solid #ddd;padding:10px;border-radius:4px;background:#fff;position:relative}
canvas{width:100%;height:200px;display:block}
.legend{text-align:center;font-size:0.9em;margin-top:5px}
.leg-item{display:inline-block;margin:0 10px}
.dot{height:10px;width:10px;border-radius:50%;display:inline-block;margin-right:5px}
#chartMsg{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#999;font-weight:bold;display:none}
</style></head><body><div class="container"><h2>Device Dashboard</h2>
<div class="chart-container"><h3 style="text-align:center;margin-top:0;color:#555;">Live History</h3><canvas id="myChart"></canvas><div id="chartMsg">Loading data...</div>
<div class="legend"><span class="leg-item"><span class="dot" style="background:#d9534f;"></span>Temp (°C)</span><span class="leg-item"><span class="dot" style="background:#0275d8;"></span>Humi (%)</span></div></div><br>)rawliteral";
...
function resizeCanvas(){const p=cvs.parentElement;cvs.width=p.clientWidth*2;cvs.height=400;cvs.style.width=p.clientWidth+'px';cvs.style.height='200px';ctx.scale(2,2);}
window.addEventListener('resize',()=>{resizeCanvas();drawGraph();});resizeCanvas();
function drawGraph(){fetch('/graphdata').then(r=>r.json()).then(d=>{
const w=cvs.clientWidth;const h=200;const bMargin=20;const gh=h-bMargin; 
ctx.clearRect(0,0,w,h);
if(!d||d.length<2){msgDiv.style.display='block';msgDiv.innerText="Not enough data ("+(d?d.length:0)+")";return;}
...
for(let i=0;i<=5;i++){let y=gh-(i*gh/5);ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.fillStyle='#999';ctx.font='10px Arial';ctx.fillText(Math.round(min+(rng*i/5)),2,y-4);}
ctx.stroke();

/* [핵심 수정] X축 10분 단위 정렬 (00, 10, 20, 30...) */
ctx.textAlign='center';
ctx.beginPath();

const startT = d[0].t;          // 시작 시간 (Timestamp)
const endT = d[d.length-1].t;   // 끝 시간
const totalT = endT - startT;   // 전체 시간 범위 (초)

// 10분 = 600초. 시작 시간 이후 가장 가까운 10분 단위 시간 찾기
// 예: 10:03분 -> 10:10분으로 올림 계산
let gridT = Math.ceil(startT / 600) * 600;

if (totalT > 0) {
    // gridT가 끝 시간보다 작거나 같을 때까지 10분(600초)씩 더하며 반복
    while(gridT <= endT) {
        // 위치 비율 계산 (현재시간 - 시작시간) / 전체시간
...
void handleGraphData() {
    ...
    // -----------------------------------------------------------
    // [핵심 변경] 1시간 범위를 커버하기 위한 '솎아내기(Step)' 로직
    // -----------------------------------------------------------
    // 목표: 1시간(3600초)을 보여주고 싶음.
    // 현재 샘플링이 약 10~12초라면, 1시간은 약 300~360개의 데이터임.
    // 하지만 우리는 안전을 위해 72개만 보낼 것임.
    
    int lookBackCount = 360; // 1시간치 데이터를 훑어봄 (10초 간격 기준)
    int maxSendCount = 72;   // 실제 전송할 데이터 개수 (안전 제한)

    // 실제 가용 데이터 확인
    int total_available = isDisplayBufferFull ? DISPLAY_MAX_SAMPLES : displayLogIndex;
    
    // 훑어볼 범위가 전체 데이터보다 크면 조정
    if (lookBackCount > total_available) lookBackCount = total_available;

    // 스텝(간격) 계산: (예: 360개를 72개로 줄이려면 5개마다 1개씩 선택)
    int step = lookBackCount / maxSendCount;
    if (step < 1) step = 1; // 최소 1

    // 실제 전송하게 될 개수
    int count = lookBackCount / step;
    
    // 시작 위치 계산 (최신 데이터 기준으로 뒤로 가기)
    int startIdx = (displayLogIndex + DISPLAY_MAX_SAMPLES - (count * step)) % DISPLAY_MAX_SAMPLES;
    if (startIdx < 0) startIdx += DISPLAY_MAX_SAMPLES; // 음수 방지
