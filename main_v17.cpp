...
void drawStatusIcons() {
    ...
    tft.drawString(modeStr, icon_x_start - 12, icon_y - 21);
    //-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // 상태 텍스트 표시-------------------------------------------------------------------------------------------------------------------------
    //String modeStr;
    if (heaterMode == AUTO) modeStr = "AUTO";
    else if (heaterMode == ON) modeStr = " ON ";
    else modeStr = " OFF";
    ...
    drawSpinningFan(fan_x, fan_y, fan_radius, fan_color);
    ...
    // 상태 텍스트 표시-------------------------------------------------------------------------------------------------------------------------
    //String modeStr;
    if (fanMode == AUTO) modeStr = "AUTO";
    else if (fanMode == ON) modeStr = " ON ";
    else modeStr = " OFF";
...
void humidifierOn() {           // AUTO, ON 상태 구분필요 // humiMode 변수값 수정금지 -> handleSetEnvironment() 여기서 수정함
  if (humidifierMode != OFF) {
    //setHumidifierHwOn();
    digitalWrite(HUMIDIFIER_PWR, HIGH);
    if (humidifierMode == ON) manualHumidifierStartTime = millis();
  }
}
...
void heaterOn() {           // AUTO, ON 상태 구분필요      // heaterMode 변수값 수정금지 -> handleSetEnvironment() 여기서 수정함
  if (heaterMode != OFF) {
    ...
    if (heaterMode == ON) manualHeaterStartTime = millis();
    //else manualHeaterStartTime = 0;
  }
}
...
void fanOn() {
  if (fanMode != OFF) {
    ...
    if (fanMode == ON) manualFanStartTime = millis();
  }
}
...
void checkHumidity() {
  ...
  if (lastHumi < humiMin && humidifierMode == AUTO)
    ...
    humidifierOn();
    //manualHumidifierStartTime = millis();      // loop() 에서 1번만 실행되도록 해야함
...
void checkTemperature() {
  ...
  if (lastTemp < tempMin && heaterMode == AUTO)
    heaterOn();
  else if (lastTemp > tempMax && heaterMode == AUTO)
    heaterOff();
  else if (heaterMode == ON)
    heaterOn();
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if (server.hasArg("humi_mode")) {
        ...
        // If humiMode changed, save it to Preferences
        if (oldHumidifierMode != humidifierMode) {
...
// heat, faning  /   heater, fan   / htr_mode  / f_mode
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  server.on("/sethumidifier", HTTP_POST, handleSetTerminal);    // 명칭변경완료: handleSetHumidifier => handleSetTerminal  (가습기,히터,팬 통합)
