 ...
 * 버그 수정완료
 * 
 * 수명 무한대: 메타데이터 저장을 아예 안 하므로, 이 로직 때문에 메모리가 죽을 일은 없습니다.
 * 플래시 메모리 보호: 잦은 기록으로 인한 보드 고장 방지 (1단계)
 * 안전장치(Failsafe): 센서가 뽑혀도 히터 과열 방지 (2단계)
 * 네트워크 복구: 공유기가 꺼져도 스스로 다시 연결 (3단계)
 ************************************************************/
...
#define GRAPH_W 210
#define GRAPH_H 130
#define NUM_LABELS 7      // X축 레이블 개수(칸갯수: n-1) Grid
...
// [수정] 내용을 비워서 아무 동작도 안 하게 만듦
void writeMeta() {
  // 이제 아무것도 저장하지 않습니다. (플래시 수명 보호)
}
...
void appendLogRecord(LogRecord& newRecord) {
  ...
  if (logFile) {
    ...
    // writeMeta();  <-- [삭제] 이 줄을 삭제하거나 주석 처리하세요.
  }
}
...
void drawStatusIcons() {
    ...
    if (humidifierMode == ON) {
      unsigned long temp = (unsigned long)((HUMIDIFIER_AUTO_OFF_MINUTES)*60) - ((millis()-manualHumidifierStartTime)/1000);
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 대신, 팬이 꺼져있을 때 정지된 회색 아이콘을 그려줍니다.
    if (!fan_on) {
    ...
    tft.drawString("      ", icon_x_start - 10, icon_y - 31);
    if (fanMode == ON) {
      unsigned long temp = (unsigned long)((FAN_AUTO_OFF_MINUTES)*60) - ((millis()-manualFanStartTime)/1000);
...
// [수정] 스프라이트에 격자 그리기 (좌표 보정 적용)
void drawGraphGrid(const TimeAxisState &axis) {
  const int Y_GRID_STEP = 20;
  for (int v = Y_MIN + Y_GRID_STEP; v <= Y_MAX; v += Y_GRID_STEP) {
    int y = map(v, Y_MIN, Y_MAX, GRAPH_H, 0); // [보정] GRAPH_Y+GRAPH_H -> GRAPH_H, GRAPH_Y -> 0
    // tft.drawLine(...) 대신 graphSprite.drawLine(...) 사용
    // X좌표도 절대좌표가 아닌 스프라이트 내부 좌표(0 ~ GRAPH_W-1) 사용
    graphSprite.drawLine(0, y, GRAPH_W - 1, y, GRID_COLOR);
  }
  for (int i = 0; i < NUM_LABELS; i++) {
    float offsetMin = (NUM_LABELS - 1 - i) * axis.labelStepMin;
    // [보정] anchorX는 절대좌표이므로 GRAPH_X를 빼서 스프라이트 상대좌표로 변환
    int x = (axis.anchorX - GRAPH_X) - (int)((offsetMin / axis.minPerPx) + 0.5f);
    if (x < 0 || x >= GRAPH_W) continue; // [보정] 범위 체크도 0 ~ GRAPH_W 기준
    graphSprite.drawFastVLine(x, 0, GRAPH_H, GRID_COLOR); // [보정] y시작점 0
  }
}
...
// [수정] 스프라이트에 데이터 선 그리기 (좌표 보정 적용)
void drawGraphData() {
    if (!timeSynced) return;
    time_t nowTs = time(nullptr);
    // int graphBottom = GRAPH_Y + GRAPH_H; // [삭제] 안 씀
    float scale = (float)GRAPH_H / (Y_MAX - Y_MIN);
    int prevTempX = -1, prevTempY = -1;
    int prevHumiX = -1, prevHumiY = -1;
    uint32_t prevValidTs = 0;
    ...
    for (int i = 0; i < record_count; i++) {
        ...
        // [보정] 절대 X좌표 계산 후 GRAPH_X를 빼서 스프라이트 좌표로 변환
        int currAbsX = GRAPH_X + GRAPH_W - (int)((float)secondsAgo / secondsPerPixel);
        int currX = currAbsX - GRAPH_X;

        if (currX < 0 || currX >= GRAPH_W) { // [보정] 범위 체크 0 ~ GRAPH_W
             prevTempX = -1; prevHumiX = -1; prevValidTs = 0; continue;
        }

        if (rec.temp != INVALID_VALUE) {
            // [보정] Y좌표 계산: GRAPH_H(바닥) 기준으로 계산
            int currTempY = GRAPH_H - (int)(((rec.temp / 10.0f) - Y_MIN) * scale + 0.5f);
            currTempY = constrain(currTempY, 0, GRAPH_H - 1); // [보정] 0 ~ GRAPH_H-1
            if (prevTempX != -1) {
                // [보정] graphSprite에 그리기
        ...
        if (rec.humi != INVALID_VALUE) {
            // [보정] Y좌표 계산
            int currHumiY = GRAPH_H - (int)(((rec.humi / 10.0f) - Y_MIN) * scale + 0.5f);
            currHumiY = constrain(currHumiY, 0, GRAPH_H - 1); // [보정]
            if (prevHumiX != -1) {
                // [보정] graphSprite에 그리기
...
void checkTemperature() {
  ...
  if (lastTemp > tempMax && fanMode == AUTO)            fanOn();
  else if (lastTemp <= tempMax && fanMode == AUTO)      fanOff();
}
...
void ntpUpdate() {
    ...
    // 3. [추가] 시간이 설정 안 된 상태에서 인터넷이 막 연결되었을 때 (재연결 직후 빠른 동기화)
    if (!timeSynced || (now - lastNtpSync >= NTP_SYNC_INTERVAL)) {
...
void setup() {
  ...
  graphSprite.createSprite(GRAPH_W, GRAPH_H); // 그래프 크기만큼 생성
...
void loop() {
  ...
  if (sysInfoDisplayUntil > 0) {
    ...
    // [수정] 여기에 있던 return; 을 삭제했습니다.
    // 이유: 이걸 지워야 정보창이 떠 있는 3초 동안에도 히터 제어와 웹서버가 멈추지 않습니다.  => 1초 주기로 디스플레이 작동으로 화면깨짐발생 (추가수정요망) 
  }
  ...
  if (nowMs - lastFanAnimMs >= 100) { // 100ms 마다 갱신
    ...
    if (fanMode != OFF && digitalRead(FAN_PIN) == HIGH) {
      ...
      int fanX = 210; 
      int fanY = ICONS_Y + 6 + 2; // 140 + 6 + 2 = 148
      // 초록색으로 회전 애니메이션 그리기
