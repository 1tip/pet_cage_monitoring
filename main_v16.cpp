...
void handleRemote() {
    ...
    // Max humidity input
    snprintf(buffer, sizeof(buffer), "<div style=\"margin-bottom: 1em;\"><label for=\"humi_max\" style=\"display: inline-block; width: 120px;\">최고 습도 (%%):</label><input type=\"number\" id=\"humi_max\" name=\"humi_max\" value=\"%d\" min=\"0\" max=\"100\" style=\"width: 80px;\"></div>", humiMax);
    server.sendContent(buffer);

    // Min humidity input
    snprintf(buffer, sizeof(buffer), "<div><label for=\"humi_min\" style=\"display: inline-block; width: 120px;\">최저 습도 (%%):</label><input type=\"number\" id=\"humi_min\" name=\"humi_min\" value=\"%d\" min=\"0\" max=\"100\" style=\"width: 80px;\"></div></fieldset>", humiMin);
    server.sendContent(buffer);

    server.sendContent(F("<fieldset><legend>가습기 제어</legend><div class=\"radio-group\"><div><input type=\"radio\" id=\"h_auto\" name=\"h_mode\" value=\"auto\" "));
    if (humidifierMode == AUTO) server.sendContent(F("checked"));
    server.sendContent(F("><label for=\"h_auto\">AUTO (자동제어)</label></div><div><input type=\"radio\" id=\"h_on\" name=\"h_mode\" value=\"on\" "));
    if (humidifierMode == ON) server.sendContent(F("checked"));
    server.sendContent(F("><label for=\"h_on\">ON (수동제어)</label></div><div><input type=\"radio\" id=\"h_off\" name=\"h_mode\" value=\"off\" "));
    if (humidifierMode == OFF) server.sendContent(F("checked"));
    server.sendContent(F("><label for=\"h_off\">OFF (수동제어)</label></div></div></fieldset><input type='submit' value='설정 저장'></form><a href=\"/dashboard\" class=\"back-link\"> &larr; Dashboard로 돌아가기</a></div>"));

    server.sendContent(F("<script>setInterval(function(){fetch('/sensordata').then(response=>response.json()).then(data=>{document.getElementById('temp_val').innerText=data.temp;document.getElementById('humi_val').innerText=data.humi;}).catch(error=>console.error('Error fetching sensor data:',error));},2000);</script></body></html>"));
