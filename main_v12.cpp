...
void drawSystemInfo() {
  ...
  tft.setCursor(10, y_offset); tft.printf("NTP Sync: %s", timeSynced ? "Synced" : "Not Synced");
  delay(3000);
...
void handleEncoderButton() {
    ...
    if (btnPrevState == HIGH && btnCurrState == LOW) { encoderPressStart = now; }
    ...
    if (btnPrevState == LOW && btnCurrState == HIGH) {
        ...
        if (pressDuration < LONG_PRESS_MS) {
            if (waitingSecondClick && (now - lastReleaseTime <= DOUBLE_CLICK_MS)) {
                drawSystemInfo(); waitingSecondClick = false;
            } else { waitingSecondClick = true; lastReleaseTime = now; }
        }
    }
    if (waitingSecondClick && (now - lastReleaseTime > DOUBLE_CLICK_MS)) {
        changeGraphMode(); waitingSecondClick = false;
    }
...
</html>)rawliteral"; server.send(200, "text/html", page); }
void handleLogin() { if (server.hasArg("username") && server.arg("username") == "cage" && server.arg("password") == "cage1234") { String page=R"rawliteral(
<!DOCTYPE html>
<html>
<head>
...
</html>)rawliteral"; server.send(200, "text/html", page); } else { handleRoot(true); } }
...
</html>)rawliteral"; preferences.begin("wifi", true); String savedSsid=preferences.getString("ssid", ""); String savedPass=preferences.getString("password", ""); preferences.end(); page.replace("%SSID_VALUE%", savedSsid); page.replace("%PASSWORD_VALUE%", savedPass); server.send(200, "text/html", page); }
...
</html>)rawliteral"; preferences.begin("wifi", true); String savedNtpMode=preferences.getString("ntpMode", "builtin1"); String savedCustomNtp=preferences.getString("ntpServer", ""); preferences.end(); page.replace("%CUSTOM_NTP_VALUE%", savedCustomNtp); if(savedNtpMode=="builtin1"){page.replace("%BUILTIN1_CHECKED%","checked");}else{page.replace("%BUILTIN1_CHECKED%","");} if(savedNtpMode=="builtin2"){page.replace("%BUILTIN2_CHECKED%","checked");}else{page.replace("%BUILTIN2_CHECKED%","");} if(savedNtpMode=="custom"){page.replace("%CUSTOM_CHECKED%","checked");}else{page.replace("%CUSTOM_CHECKED%","");} server.send(200, "text/html", page); }
...
</html>)rawliteral"; server.send(400,"text/html",page); return;} String pass=server.arg("password"); preferences.begin("wifi",false); preferences.putString("ssid",ssid); preferences.putString("password",pass); preferences.end(); String page=R"rawliteral(
<!DOCTYPE html>
<html>
<head>
    <title>Saved</title>
...
</html>)rawliteral"; server.send(200,"text/html",page); delay(2000); ESP.restart(); } else { server.send(400, "text/plain", "400: Invalid Request"); } }
...
</html>)rawliteral"; server.send(200,"text/html",page); delay(2000); ESP.restart(); }
...
void handleDownloadLog() {
    ...
    for (uint32_t i = 0; i < records_to_read; i++) {
        ...
        if (logFile.read((uint8_t*)&rec, sizeof(LogRecord)) == sizeof(LogRecord)) {
            ...
            String line = String(timeStr) + "," + String(rec.temp / 10.0f, 1) + "," + String(rec.humi / 10.0f, 1) + "," + String(rec.ts) + "\n";
