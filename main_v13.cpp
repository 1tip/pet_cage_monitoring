...
<body>
    <div class="container">
        ...
        <form method='POST' action='/sethumidifier'>
            <fieldset>
                <legend>습도 범위 설정 (자동 모드)</legend>
                <p style="text-align: center; font-weight: bold; color: #337ab7;">
                  현재 온도: %CURRENT_TEMP% &deg;C / 현재 습도: %CURRENT_HUMI% %
                </p>
            ...
            <fieldset>
                ...
                <div class="radio-group">
                    <div><input type="radio" id="h_auto" name="h_mode" value="auto" %AUTO_CHECKED%><label for="h_auto">자동제어(AUTO)</label></div>
                    <div><input type="radio" id="h_on" name="h_mode" value="on" %ON_CHECKED%><label for="h_on">수동제어 ON</label></div>
                    <div><input type="radio" id="h_off" name="h_mode" value="off" %OFF_CHECKED%><label for="h_off">수동제어 OFF</label></div>
...
void handleSetHumidifier() {
    ...
    if (server.hasArg("h_mode")) {
        ...
        if (mode == "on") {
            humidifierOn(); // 조건 없이 ON
        } else if (mode == "off") {
            humidifierMode = OFF; // OFF 모드로 설정
            humidifierOff();      // 즉시 OFF
        } else { // "auto"
            humidifierMode = AUTO;
            humidifierOff(); // 자동 모드 시작 시 OFF 상태에서 시작
        }
    }
